...
export default function GlobalAICopilotPage() {
  ...
  const handleSendMessage = async (customText?: string) => {
    ...
    const userMsgText = textToSend || (attachedFile ? `Uploaded document: ${attachedFile.fileName}` : "");
    ...
    // Check if user is describing a full new event to plan
    const isPlanIntent =
      /create event|plan event|organize|schedule an event|cultural night|hackathon|conference|seminar|from \d|auditorium|artists|anchors/i.test(
        userMsgText
      ) && userMsgText.split(/\s+/).length >= 5;
    ...
    const requestType = isImageIntent
      ? "image_generate"
      : isPlanIntent || (extractedTextToSend && /event|schedule|speaker/i.test(extractedTextToSend))
      ? "event_builder"
      : "copilot";
      ...
      if (requestType === "event_builder") {
        ...
        if (plan && plan.name) {
          ...
          addMessageToConversation(targetConvId, {
            id: `asst_${Date.now()}`,
            role: "assistant",
            content: `I've prepared a complete structured Event Plan for "${plan.name}". You can review, edit details, and confirm creation in the preview below.`,
            createdAt: Date.now(),
            plan,
        ...
        let parsed: { reply?: string; action?: AIAction | null } = {};
        try {
        ...
        if (candidateAction && candidateAction.type) {
    ...
    <div className="flex h-[calc(100vh-4rem)] overflow-hidden bg-[#070B14] text-slate-100">
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-[#070B14]">
          ...
          {messages.length === 0 && (
            <div className="max-w-2xl mx-auto my-12 text-center">
              ...
              <p className="text-sm text-slate-400 mb-8 max-w-md mx-auto">
                Ask questions, upload event schedule PDFs/DOCs, generate stage scripts & posters, or operate live events seamlessly.
              </p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-left">
                ...
                <button
                  onClick={() => handleSendMessage("Create a futuristic tech keynote script for our chief guest")}
                  className="p-3.5 rounded-xl border border-slate-800 bg-slate-900/60 hover:bg-slate-800/80 transition-all text-left group"
                >
                  <div className="text-xs font-semibold text-amber-400 mb-1 group-hover:text-amber-300">🎙️ Script Generation</div>
                  <div className="text-xs text-slate-300">&ldquo;Create a keynote script for our chief guest&rdquo;</div>
                </button>
              </div>
            </div>
          )}
                  ...
                  <div className="p-4 rounded-xl border border-blue-500/30 bg-blue-950/20 space-y-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-semibold text-white text-sm">{msg.plan.name}</span>
                      </div>
                      <span className="text-[11px] px-2 py-0.5 rounded-md bg-blue-500/20 text-blue-300 font-medium">
                        {msg.plan.type}
                      </span>
                    </div>

                    <p className="text-xs text-slate-300">{msg.plan.description}</p>
        ...
        <div className="p-4 border-t border-slate-800/80 bg-slate-950/60">
          ...
          {attachedFile && (
            <div className="max-w-4xl mx-auto mb-2 flex items-center justify-between p-2 px-3 rounded-xl bg-slate-900 border border-slate-800 text-xs text-slate-200">
              <div className="flex items-center gap-2 min-w-0">
                <FileText className="w-4 h-4 text-blue-400 shrink-0" />
                <span className="font-semibold truncate">{attachedFile.fileName}</span>
                <span className="text-[10px] text-slate-400">({Math.round(attachedFile.fileSize / 1024)} KB)</span>
              </div>
              <button
          ...
          >
            ...
            <button
              ...
              title="Upload Schedule PDF, DOCX, PPTX, or Image"
            ...
            >
              {isFileProcessing ? <Loader2 className="w-4 h-4 animate-spin text-blue-400" /> : <Paperclip className="w-4 h-4" />}
            </button>
            ...
            <input
              ...
              placeholder={attachedFile ? `Ask about ${attachedFile.fileName}...` : "Ask anything about your events, upload a schedule, generate scripts or posters..."}
