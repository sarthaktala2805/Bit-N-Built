...
export const MobileNav: React.FC = () => {
  ...
  return (
    <>
      {/* Mobile Top Header */}
      <div className="md:hidden sticky top-0 z-40 bg-slate-950/90 border-b border-slate-800/80 px-3.5 py-2.5 flex items-center justify-between backdrop-blur-md gap-2">
        <div className="flex items-center gap-2 min-w-0 flex-1">
          ...
          {activeEvent && (
            <Link
              ...
              className="text-[11px] font-medium bg-slate-900/90 border border-slate-800 px-2 py-0.5 rounded-full text-slate-300 truncate max-w-[90px] xs:max-w-[120px] sm:max-w-[160px] hover:border-slate-700 transition"
            >
              {activeEvent.name}
            </Link>
          )}
        ...
        <button
          onClick={logout}
          title="Sign out"
          className="flex items-center gap-1.5 text-xs text-slate-400 hover:text-red-400 transition-colors px-2 py-1 rounded-lg hover:bg-slate-900 shrink-0"
        >
